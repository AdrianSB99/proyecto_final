...
app.post('/registrar', upload.single('file'), (req, res) => {
    ...
    conexion.query(sql, [Nom, Con, Cor, Sem, Usu, Cont, Des, picture], function(err, result) {
        if (err) {
          ...
          res.send('<script>alert("' + mensaje + '"); window.location.href="/Aregistro";</script>');
        } else {
...
app.post('/registrarprofe', upload.single('file'), (req, res) => {
  ...
  conexion.query(sql, [Nom, Con, Cor, Sem, Usu, Cont, Des, picture], function(err, result) {
      if (err) {
        var mensaje = "El nombre de usuario ya está en uso.";
        res.send('<script>alert("' + mensaje + '"); window.location.href="/Dusuarios";</script>');
      } else {
        if (result.affectedRows > 0) {
          var mensaje = "Registro exitoso";
          res.send('<script>alert("' + mensaje + '"); window.location.href="/Dusuarios";</script>');
...
app.get('/', (req,res)=>{
    res.render('AinicioAlu');
});

//SECCION PROYECTOS
app.get('/', (req,res)=>{
    res.render('CproyectosAlu');
});

app.get('/', (req,res)=>{
  res.render('CproyectosPro');
});
  ...
  app.post('/registroproy', (req, res) => {
    ...
    conexion.query(sql, [Nom, Des, url, Prop], function(err, result) {
        if (err) {
          ...
          res.send('<script>alert("' + noproy + '"); window.location.href="/EagregarProyecto";</script>');
        } else {
          if (result.affectedRows > 0) {
            var siproy = "Registro exitoso";
            res.send('<script>alert("' + siproy + '"); window.location.href="/CproyectosAlu";</script>');
          } else {
            var noproy2 = "No se pudo insertar el registro";
            res.send('<script>alert("' + noproy2 + '"); window.location.href="/Eagregarproyecto";</script>');
          }
        }
      });
});
  ...
  app.post('/actualizaruser', (req, res) => {
    ...
    conexion.query(query, [campo, nuevoValor, noControl], (error, resultado) => {
      if (error) {
        ...
        res.send('<script>alert("' + mensajeActualizar + '"); window.location.href="/Dusuarios";</script>');
      }
    });
  });
  ...
  app.post('/actualizarfoto', upload.single('file'), (req, res) => {
    ...
    app.post('/eliminaruser', (req, res) => {
        ...
        conexion.query(query, [id], (error, resultado) => {
          ...
          res.send('<script>alert("' + mensajeeliminar + '"); window.location.href="/Dusuarios";</script>');
          }
        });
      });
    ...
    app.post('/eliminarproy', (req, res) => {
      ...
      conexion.query(query, [id], (error, resultado) => {
        ...
        res.send('<script>alert("' + mensajeeliminar + '"); window.location.href="/CproyectosPro";</script>');
        }
      });
    });
